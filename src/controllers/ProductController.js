...
class ProductController {
  static async getProducts(req, res) {
    try {
      let defaultLimit = req.query.limit
        ? parseInt(req.query.limit)
        : parseInt(process.env.PAGING_LIMIT);

      let defaultPage = req.query.page
        ? parseInt(req.query.limit)
        : parseInt(process.env.PAGING_PAGE);
  ...
  static async getProductsById(req, res) {
    try {
      const productsRes = await Product.findByPk(req.params.id, {
        include: [{ model: User }],
      });
      if (!productsRes) throw Error("data product not found");
  ...
  static async deleteProductsById(req, res) {
    try {
      ...
      if (!productsRes) throw Error("delete data by id product not found");
